...
| **Pre-collected brief** | `src/main/engine/precollect.ts`, best-effort | runbook steps 1–3 run *as code* before the session: onset window normalized to epoch · Grafana deploy annotations in that window · Loki error-rate deltas (in-window vs the same window 24h earlier) · **Kubernetes signals via the Grafana Prometheus datasource** (pod restarts, OOMKills, deploy-generation changes, peak unavailable replicas from kube-state-metrics — durable history that live `kubectl get events` can't give, and no cluster credentials needed). The agent audits it and starts at step 4 instead of doing the epoch math and metric lookups by hand. Absent Grafana/Prometheus or on any failed query it degrades to a note and the agent does that part itself |
...
2. **Read-only tools → auto-allow** (Read/Grep/Glob/WebFetch/Task/…, the in-process `mcp__memory__*` tools — the whole prefix trusted, read-only by construction — plus `Bash` commands that pass a deny-by-default read-only regex: `git log` yes, `git push` never, smuggling like `$(...)` or `;` rejected). `kubectl` is classified by a tokenizer that tolerates leading `--context`/`-n`/etc. targeting flags before a read verb (`get`/`describe`/`logs`/`rollout history`…) — so per-service live cluster reads run without a prompt, while write verbs and auth-changing flags (`--as`, `--token`) still gate. Each service's `--context`/namespace comes from the registry.
